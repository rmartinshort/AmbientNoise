...
//	VELOCITIES allVel[MAX_COR];
	...
	/* set distance and snt thresholds */
	/* See Bensen (2007) for more details */
	//snrThresshold = 30;
	...
	while (fgets(buff,100,sacListFile)) {
		...
		if (skip_flag == 0) {
			...
			if (read_ftan(ftanFileName,&tempDispersion) == 0) {
				skip_flag = 1;
				for (i=0; i<nPer; i++) {
					allVel[l].ph[i] = NAN_VALUE;
				}
	...
	for (i=0; i<nPer; i++) {
		...
		sprintf(phOutName,"%s/data%0.0fs_ph.txt",outputDirectory,per);
