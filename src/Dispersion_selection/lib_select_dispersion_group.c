...
//trick to read the group velocity rather than the phase velocity
int read_ftan_gp(char *fname, DISPERSION *disp) {
        int i,index;
        ...
        while (fgets(buff,100,ff)) {
               sscanf(buff,"%d %f %f %s",&index, &central_period, &tmp_period, isnan);
                if (strcmp(isnan,"nan")) {
                        sscanf(buff,"%d %f %f %f %f %f %f\n",&index, &central_period, &disp->per[i], &disp->ph[i], &dumgp, &amp, &snr);
                } else {
