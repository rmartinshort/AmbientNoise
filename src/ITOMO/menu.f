...
c END TYPE definitions ++++++++++++++++++++++++++++++++++
 ...
 1	print *,'		TOMO SETTINGS:'
	...
	print *,'19.) produce resolution analysis map?..(toggle)...',iresol
	print *,'20.) Make response map for some points?(toggle)...',ipoint
	if(char.eq.'v') return
...
1003    format(A55,2F6.1)
	...
	elseif(c(1:lc).eq.'6') then
	 ...
	 alph1(4)=alph1(2)
	 print *,'WARNING !!! MAX sigma is less than initial, MAX=initial'
	 endif
	...
	elseif(c(1:lc).eq.'15') then
	 print *,'enter pow, sign is always +'
	 read(*,*) pow
	...
	elseif(c(1:lc).eq.'18') then
	 ...
	 icoout=1
        ...
        elseif(c(1:lc).eq.'19') then
         if(iresol.eq.1)then
         iresol=0
         else
         iresol=1
         endif
        elseif(c(1:lc).eq.'20') then
         if(ipoint.eq.1)then
          ipoint=0
        else
          ipoint=1
