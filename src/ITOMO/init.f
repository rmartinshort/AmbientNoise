...
c TYPE definitions ++++++++++++++++++++++++++++++++++++++
      ...
      CHARACTER*1  srejd,scoord,scoout,sresol,spoint
      CHARACTER*1 TEX
...
C-----to start computations---------------------------
       ...
       go to 1
100    outfile=namout(1:lo)//'_'//period(1:lp)//'.1'
     ...
     * //'.'//'azi',status='UNKNOWN')
       ...
       if(lresol) then
c* UNCOMM       open(unit=17,file=namout(1:lo)//'_'//period(1:lp)
c* UNCOMM     * //'.'//'mat',form='unformatted',status='UNKNOWN')
         open(unit=22,file=namout(1:lo)//'_'//period(1:lp)
     *   //'.'//'rea',status='UNKNOWN')
       endif
       if(lresid)
